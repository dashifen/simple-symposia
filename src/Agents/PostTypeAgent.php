...
class PostTypeAgent extends AbstractPluginAgent
{
  ...
  public function initialize(): void
  {
    if (!$this->isInitialized()) {
      ...
      $this->addFilter('post-type-registration-thumbnail-label', 'filterSpeakerThumbnailLabel', 10, 2);
    }
  }
  ...
  /**
   * filterSpeakerThumbnailLabel
   *
   * Changes the name of a speaker's featured image to their photograph.
   *
   * @param string $label
   ...
   * @return string
   */
  protected function filterSpeakerThumbnailLabel(string $label, string $singular): string
  {
    return $singular === 'Speaker'
      ? 'photograph'
      : $label;
  }
}
